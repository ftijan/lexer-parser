...
namespace Parser
{
    public class Parser
    {
        ...
        public DslQueryModel Parse(IList<DslToken> tokens)
        {
        ...
        private void LoadSequenceStack(IList<DslToken> tokens)
        {
        ...
        private DslToken ReadToken(TokenType tokenType)
        {
            ...
            {
                throw new DslParserException($"Expected {tokenType.ToString().ToUpper()} but found: {_lookaheadFirst.Value}");
            }
        ...
        private void DateCondition()
        {
            ...
            _queryModel.DateRange.From = DateTime.ParseExact(ReadToken(TokenType.DateTimeValue).Value, timestampFormat, CultureInfo.InvariantCulture);
            DiscardToken(TokenType.DateTimeValue);
            DiscardToken(TokenType.And);
            _queryModel.DateRange.To = DateTime.ParseExact(ReadToken(TokenType.DateTimeValue).Value, timestampFormat, CultureInfo.InvariantCulture);
            DiscardToken(TokenType.DateTimeValue);
            DateConditionNext();
        }
